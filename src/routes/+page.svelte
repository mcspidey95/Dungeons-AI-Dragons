<script>
	...
	async function typeStoryText() {
		let typingIndex = 0;
		let isStoryEnd = false;
		let storyLines = storyFull
			.split('\n')
			.map((line) => line.trim())
			.filter((line) => line); // Clean and remove empty lines
...
{#if back1}
	<img
		id="back-button1"
		class={isBack1 ? 'back-button1' : 'back-button-hidden'}
		src={back1}
		on:click={() => handleBackClick1()}
	/>
{/if}
...
{#if showCharacterSelection}
	<div class="card-container">
		...
		<!-- Card 4 (Custom) -->
		<div
			class="character-card custom-card"
			on:click={() => {
				showTextarea = true;
				showCharacterSelection = false;
				isBack1 = false;
...
{#if showTextarea}
	<img
		class={isBack ? 'back-button' : 'back-button-hidden'}
		src={back}
	...
	<div class="textarea-wrapper {showCharacterSheet ? 'character-sheet' : ''}">
		{#if showCharacterSheet}
			<div class="character-input character-sheet">
				...
				<br />
				<span class="sheet-feature" id="grey"
					><p id="white">Name</p>
					<span class="sheet-description" id="sheet-name">{sheetName}</span></span
				>
				<span class="sheet-feature" id="grey"
					><p id="white">Class</p>
					<span class="sheet-description" id="sheet-name1">{sheetClass}</span></span
				>
				<span class="sheet-feature" id="grey"
					><p id="white">Race</p>
					<span class="sheet-description" id="sheet-name1">{sheetRace}</span></span
				>
				<br />
				<span class="sheet-feature" id="grey"
					><p id="white">
						Strength<span class="sheet-num" id={sheetStrNum > 10 ? 'green' : 'red'}
							>({sheetStrNum})</span
						>
					</p>
					<span class="sheet-description">{sheetStr}</span></span
				>
				<span class="sheet-feature" id="grey"
					><p id="white">
						Weakness<span class="sheet-num" id={sheetWeakNum > 10 ? 'green' : 'red'}
							>({sheetWeakNum})</span
						>
					</p>
					<span class="sheet-description">{sheetWeak}</span></span
				>
				<span class="sheet-feature" id="grey"
					><p id="white">
						Fear<span class="sheet-num" id={sheetFearNum > 10 ? 'green' : 'red'}
							>({sheetFearNum})</span
						>
					</p>
					<span class="sheet-description">{SheetFear}</span></span
				>
				<span class="sheet-feature" id="grey"
					><p id="white">
						Agility<span class="sheet-num" id={sheetAgilityNum > 10 ? 'green' : 'red'}
							>({sheetAgilityNum})</span
						>
					</p>
					<span class="sheet-description">{sheetAgility}</span></span
				>
				<span class="sheet-feature" id="grey"
					><p id="white">
						Morality<span class="sheet-num" id={sheetMoralityNum > 10 ? 'green' : 'red'}
							>({sheetMoralityNum})</span
						>
					</p>
					<span class="sheet-description">{sheetMorality}</span></span
				>
				<span class="sheet-feature" id="grey"
					><p id="white">
						Fame<span class="sheet-num" id={sheetFameNum > 10 ? 'green' : 'red'}
							>({sheetFameNum})</span
						>
					</p>
					<span class="sheet-description">{sheetFame}</span></span
				>
				<br />
				<span class="sheet-feature" id="grey"
					><p id="white">Special Abilities</p>
					<span class="sheet-description">{sheetAbility}</span></span
				>
				<br />
				<span class="sheet-feature" id="grey"
					><span class="sheet-description">{sheetStory}</span></span
				>
			</div>
		{:else}
			<textarea
